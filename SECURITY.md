...
- The browser exchanges the master token for an independent random session cookie and CSRF proof. Only credential digests remain in process memory. The HttpOnly, host-only, `SameSite=Strict` cookie has an eight-hour maximum age. The proof and a public session identifier stay in origin-scoped browser storage so same-origin tabs use the pair that matches the shared cookie. Because the supported local UI uses HTTP, the cookie is not marked `Secure`; host loopback and the Compose ingress remain required boundaries.
- Browser mutations require the derived cookie, the separate proof, and an Origin exactly equal to `http://localhost:8080` or `http://127.0.0.1:8080`. A cookie cannot take the CLI path, and a derived browser credential cannot be replayed as the master-token header. Command-line mutations omit Origin and present the master token only through `X-RedDock-Operator-Token`. CORS remains closed and mutations remain JSON-only.
- Browser sessions expire after eight hours or 30 idle minutes, are limited to eight active sessions, and disappear on restart or master-token mismatch. Mutation admission is limited to 120 requests per minute per process. Token unlock attempts are limited to 10 per minute per process. These availability controls are not database-wide or multi-process limits.
