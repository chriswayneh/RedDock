...
const onError = vi.fn();
const onBack = vi.fn();
const lists = ["scope", "assetPage", "servicePage", "observationPage", "discoveryPage", "detectionPage", "findingPage", "validationPage"] as const;
...
describe("workspace request ownership", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.spyOn(api, "scope").mockResolvedValue([]);
    for (const name of lists.filter((name) => name !== "scope")) {
      vi.spyOn(api, name).mockResolvedValue({ items: [], total: 0 });
    }
  });
  afterEach(() => { cleanup(); vi.restoreAllMocks(); vi.useRealTimers(); });

  it.each<[WorkspaceTab, string[]]>([
    ["Scope", ["scope"]], ["Discovery", ["scope", "discoveryPage"]],
    ["Assets", ["assetPage"]], ["Services", ["servicePage"]],
    ["Observations", ["observationPage"]], ["Detection", ["observationPage", "detectionPage"]],
    ["Validation", ["findingPage", "validationPage"]], ["Runs", ["discoveryPage"]], ["Findings", ["findingPage"]],
  ...
  it("polls running validation every two seconds, then stops at completion", async () => {
    vi.mocked(api.validationPage).mockResolvedValueOnce({ items: [validation], total: 1 }).mockResolvedValue({ items: [{ ...validation, status: "completed" }], total: 1 });
    await show("Validation");
    await act(async () => { await vi.advanceTimersByTimeAsync(1999); });
    expect(api.validationPage).toHaveBeenCalledTimes(1);
    await act(async () => { await vi.advanceTimersByTimeAsync(1); });
    expect(api.validationPage).toHaveBeenCalledTimes(2);
    await act(async () => { await vi.advanceTimersByTimeAsync(6000); });
    expect(api.validationPage).toHaveBeenCalledTimes(2);
  });
  ...
  it("polls active discovery and stops after completion", async () => {
    ...
    vi.mocked(api.discoveryPage).mockResolvedValueOnce({ items: [run], total: 1 }).mockResolvedValue({ items: [{ ...run, status: "completed" }], total: 1 });
    await show("Runs");
    await act(async () => { await vi.advanceTimersByTimeAsync(2000); });
    expect(api.discoveryPage).toHaveBeenCalledTimes(2);
    await act(async () => { await vi.advanceTimersByTimeAsync(6000); });
    expect(api.discoveryPage).toHaveBeenCalledTimes(2);
  });

  it("does not poll a pending human approval or after unmount", async () => {
    vi.mocked(api.validationPage).mockResolvedValue({ items: [{ ...validation, status: "pending" }], total: 1 });
    await show("Validation");
    await act(async () => { await vi.advanceTimersByTimeAsync(6000); });
    expect(api.validationPage).toHaveBeenCalledTimes(1);
    cleanup();
    vi.mocked(api.validationPage).mockResolvedValue({ items: [validation], total: 1 });
    await show("Validation");
    cleanup();
    await act(async () => { await vi.advanceTimersByTimeAsync(6000); });
    expect(api.validationPage).toHaveBeenCalledTimes(2);
