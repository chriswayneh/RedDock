...
The runner owns every step that can reject invalid data. It builds the snapshot, validates output, computes identity, reconciles known state, resolves what is no longer reproduced, and writes evidence. If a detector returns malformed data, its whole result is discarded. A failed detector resolves nothing because not running is not evidence that an issue went away.
