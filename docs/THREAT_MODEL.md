...
| P1 | **Design hypothesis:** a future browser or CSRF token is stolen, fixed, replayed, or survives role revocation | Authenticated cookie routes exist; account impersonation or unauthorized mutation | Independent 256-bit tokens, hash-only storage, strict token shape, eight-hour expiry, targeted and membership-wide revocation, active user/membership checks, constant-time CSRF digest comparison, and explicit inactive-record cleanup | Add secure cookie issuance/rotation, exact origin checks, session limits and audit events before enabling server mode | `backend/app/session_auth.py:14-16`, `backend/app/session_auth.py:47-191`, `backend/tests/test_session_auth.py:21-137` |
