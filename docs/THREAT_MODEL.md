...
| Future server mode | Accounts, sessions, tenant data | Identity tables, local ownership, a deny-by-default permission contract, enforced route manifest, request-scoped organization context, a dormant bounded hash-only session lifecycle, and structured security-event storage exist, but no networked authorization mode exists | Local requests resolve to the reserved owner; unsupported modes fail startup; server mode remains unavailable until every requirement is present | `backend/app/authorization.py:7-201`, `backend/app/authorization_dependencies.py:21-70`, `backend/app/session_auth.py:124-164`, `backend/app/security_audit.py:13-80`, `backend/app/config.py:90-99` |
...
| P1 | **Design hypothesis:** a future browser or CSRF token is stolen, fixed, replayed, or survives role revocation | Authenticated cookie routes exist; account impersonation or unauthorized mutation | Independent 256-bit tokens, hash-only storage, strict token shape, eight-hour expiry, targeted and membership-wide revocation, active user/membership checks, constant-time CSRF digest comparison, an eight-active-session cap, explicit inactive-record cleanup, and atomic issue/logout audit events | Add secure cookie issuance/rotation, exact origin checks, and audits for remaining privileged flows before enabling server mode | `backend/app/session_auth.py:15-16`, `backend/app/session_auth.py:49-248`, `backend/tests/test_session_auth.py:23-162` |
