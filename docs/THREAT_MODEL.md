...
| Future server mode | Accounts, sessions, tenant data | Identity tables, local ownership, a deny-by-default permission contract, enforced route manifest, request-scoped organization context, a dormant authentication coordinator, a bounded hash-only session lifecycle, exact HTTPS origin policy, host-bound secure-cookie contract, strict one-hop proxy metadata validation, and structured security-event storage exist, but no networked authorization mode exists | Local requests resolve to the reserved owner; public-origin configuration and unsupported modes fail startup; server mode remains unavailable until every requirement is integrated. Uvicorn does not interpret proxy headers implicitly. No route or UI invokes the coordinator. | `backend/app/authentication.py`, `backend/app/authorization.py`, `backend/app/authorization_dependencies.py`, `backend/app/session_auth.py`, `backend/app/browser_security.py`, `backend/app/trusted_ingress.py`, `backend/app/security_audit.py`, `backend/app/config.py`, `backend/tests/test_authentication.py` |
...
| P2 | Oversized results, dense state, or concurrent provider fetches exhaust shared resources | High-volume authorized inputs or future sign-in traffic; availability loss | Fixed run, response, snapshot, edge, and export bounds; one application-owned OIDC cache serializes provider fetches; isolated, database-serialized, global-first throttling bounds cross-worker admission and stored keyed client buckets | Add metrics and expose the already throttled coordinator only through a reviewed auth adapter; preserve PostgreSQL global, subject, reset, cleanup, and one-use callback race coverage | `backend/app/authentication.py`, `backend/app/oidc.py`, `backend/app/rate_limits.py`, `backend/tests/test_authentication.py`, `backend/tests/test_postgres.py`, `backend/app/intelligence/providers.py:92-99` |
