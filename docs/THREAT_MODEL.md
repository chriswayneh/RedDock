...
| P2 | Oversized results or dense state exhausts shared resources | High-volume authorized inputs; availability loss | Fixed run, response, snapshot, edge, and export bounds | Add throttling and metrics; exercise PostgreSQL concurrency | `backend/app/config.py:143-196`, `backend/app/intelligence/providers.py:92-99` |
