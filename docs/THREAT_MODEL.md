...
| P1 | **Design hypothesis:** a future browser or CSRF token is stolen, fixed, replayed, or survives role revocation | Authenticated cookie routes exist; account impersonation or unauthorized mutation | Independent 256-bit tokens, hash-only storage, strict token shape, eight-hour expiry, explicit revocation field, active user/membership checks, constant-time CSRF digest comparison | Add secure cookie issuance/rotation/logout, exact origin checks, session limits and audit events before enabling server mode | `backend/app/session_auth.py:14-16`, `backend/app/session_auth.py:47-130`, `backend/tests/test_session_auth.py:18-84` |
