...
RBAC, optional PostgreSQL, scaling work, release automation, ARM64 support, and production hardening. The persistence checkpoint now includes a validated Alembic baseline, packaged driver, a private pinned [PostgreSQL Compose profile](docs/POSTGRESQL.md), mounted database/provider secrets, and real-server migration/CRUD CI. The identity-schema checkpoint creates organizations, OIDC-keyed user profiles, memberships, hash-only session storage, and non-null organization ownership while preserving the account-free local workflow; it does not yet enable authentication. The default no-LLM package and optional AMD64/ARM64 Ollama + Qwen3.5 4B bundle are explicit. The source-backed [threat model](docs/THREAT_MODEL.md) and [identity/tenancy ADR](docs/adr/0013-production-identity-and-tenancy.md) define the non-negotiable boundary between backward-compatible loopback local mode and a future fail-closed authenticated server mode. Complete when sessions, OIDC, authorization, administration, deployment, backup/restore, scaling, and operational requirements are implemented, documented, and validated end to end.
