...
**Next after v0.8.0: Phase 8 — Production polish.** The PostgreSQL validation profile, migration CI, backward-compatible identity/tenancy schema, least-privilege role/permission contract, and exhaustive API permission manifest are complete. Shared mode is explicitly rejected and authentication is not enabled yet: session resolution, permission enforcement, OIDC, administration, scaling, release automation, full ARM64 validation, backup/restore, and production deployment hardening remain planned. See the [roadmap](ROADMAP.md) for the complete phased plan.
